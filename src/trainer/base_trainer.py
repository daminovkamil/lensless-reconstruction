...
class BaseTrainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(
            ...
            seen_images += self._get_batch_size(batch)
            # log current results
            if batch_idx % self.log_step == 0:
                self.train_metrics.update("grad_norm", self._get_grad_norm())
                now = time.perf_counter()
                elapsed = max(now - last_log_time, 1e-12)
                images_per_sec = (seen_images - last_log_images) / elapsed
                self.writer.set_step((epoch - 1) * self.epoch_len + batch_idx)
                self.logger.debug(
                    "Train Epoch: {} {} Loss: {:.6f} Img/s: {:.2f}".format(
                        epoch,
                ...
                self.writer.add_scalar("train_images_per_sec", images_per_sec)
    ...
    def _get_batch_size(batch):
        if "lensless" in batch and torch.is_tensor(batch["lensless"]):
            return int(batch["lensless"].shape[0])
    ...
    def _final_test(self):
        """Evaluate the held-out test set once, after training, on the best checkpoint."""
        if "test" not in self.evaluation_dataloaders:
            return
        best_path = self.checkpoint_dir / "model_best.pth"
        if best_path.exists():
            self._from_pretrained(best_path)
        test_logs = self._evaluation_epoch(
            self.epochs, "test", self.evaluation_dataloaders["test"]
        )
        self.logger.info("Final test on the best checkpoint:")
        for name, value in test_logs.items():
            self.logger.info(f"    test_{name:15s}: {value}")
